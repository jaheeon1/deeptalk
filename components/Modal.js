...
export function Modal() {
	...
	const closeModal = () => {
		reset();
		dispatch(setTalkModalOpen(false));
		dispatch(setSelectedTalk(undefined));
	};
	...
	const onSubmitHandler = (data) => {
		...
		if (state.selectedTalk) {
			dispatch(
				updateTalk({
					_id: state.selectedTalk._id,
					...data,
				})
			);
		} else {
			dispatch(addTalk(data));
		}
	};

	useEffect(() => {
		if (state.selectedTalk) {
			setValue("name", state.selectedTalk.name);
			setValue("email", state.selectedTalk.email);
			setValue("address", state.selectedTalk.address);
			setValue("phone", state.selectedTalk.phone);
		}
	}, [state.selectedTalk, setValue]);
						...
						>
							...
							<div className="form__action">
								...
								<button className="btn btn__primary btn__icon" type="submit">
									<CheckSVG /> {state.selectedTalk ? "Update" : "Submit"}
								</button>
							</div>
						</form>
					</div>
