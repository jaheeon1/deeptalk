...
export function TeacherModal() {
	...
	const closeModal = () => {
		reset();
		dispatch(setTeacherModalOpen(false));
		dispatch(setSelectedTeacher(undefined));
	};
	...
	useEffect(() => {
		if (state.selectedTeacher) {
			setValue("name", state.selectedTeacher.name);
			setValue("email", state.selectedTeacher.email);
			setValue("address", state.selectedTeacher.address);
			setValue("phone", state.selectedTeacher.phone);
		}
	}, [state.selectedTeacher, setValue]);
						...
						<header className="header modal__header">
							<h1 className="header__h2">
								{state.selectedTeacher ? (
									<>
										Edit <span>Teacher</span>
									</>
								) : (
									<>
										Add <span>Teacher</span>
									</>
								)}
							</h1>
							<button
						...
						>
							...
							<div className="form__action">
								...
								<button className="btn btn__primary btn__icon" type="submit">
									<CheckSVG /> {state.selectedTeacher ? "Update" : "Submit"}
								</button>
							</div>
						</form>
					</div>
